...
public sealed class BloscCodec : IZarrCodec
{
        ...
        Span<byte>         dest)
    {
        ...
        var compressed = frame.AsSpan(streamOffset, csize);

        if (csize == dest.Length)
        {
            compressed.CopyTo(dest);   // stored raw (incompressible)
        }
        else
        {
            DecompressBlock(codec, compressed, dest);
        }
    ...
    private byte[] CompressBlockToStreams(ReadOnlySpan<byte> block, int nSplits)
    {
        ...
        var splitSize    = shuffled.Length / nSplits;
        var streamParts  = new byte[nSplits][];

        for (int splitIdx = 0; splitIdx < nSplits; splitIdx++)
        {
            var segment    = shuffled.AsSpan(splitIdx * splitSize, splitSize).ToArray();
            var compressed = CompressBlock(segment, splitSize);
